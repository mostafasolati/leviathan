...
var (
	mobileOSRegex = regexp.MustCompile(
		`(?i)(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|` +
			`compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|` +
			`lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|` +
			`palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|` +
			`symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino`,
	)
	mobileModelRegex = regexp.MustCompile(
		`(?i)1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|` +
...
// IsMobileWeb matches a User-Agent header to detect mobile browsers.
func IsMobileWeb(userAgent string) bool {
	if len(userAgent) < 4 {
		return false
	}
	return mobileOSRegex.MatchString(userAgent) || mobileModelRegex.MatchString(userAgent[:4])
}
